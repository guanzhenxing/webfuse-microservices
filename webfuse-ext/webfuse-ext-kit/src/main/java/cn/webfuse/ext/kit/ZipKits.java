...
/**
 * From org.itkk.udf.core.utils.ZipUtil
 * Author: wangkang
 */
public class ZipKits {
     ...
     * @param encoding 编码
     */
    public ZipKits(String encoding) {
    ...
    private void decompressZip(File file, String saveFileDir) throws IOException {
        try (InputStream is = new FileInputStream(file); ZipArchiveInputStream zais = new ZipArchiveInputStream(is, this.encoding)) {
    ...
    private void compressFiles2Zip(File file, ZipArchiveOutputStream zaos) throws IOException {
        ...
        if (file != null) {
            ...
            try (InputStream is = new BufferedInputStream(new FileInputStream(file), ioBuffered)) {
