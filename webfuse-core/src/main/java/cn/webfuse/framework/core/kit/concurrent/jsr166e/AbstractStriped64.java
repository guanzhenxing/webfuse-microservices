...
package cn.webfuse.framework.core.kit.concurrent.jsr166e;
 ...
 */
public abstract class AbstractStriped64 extends Number {
    /*
     ...
     */
    static final ThreadLocal<int[]> THREAD_HASH_CODE = new ThreadLocal<>();
    ...
    final void retryUpdate(long x, int[] hc, boolean wasUncontended) {
        int h;
        if (hc == null) {
            THREAD_HASH_CODE.set(hc = new int[1]); // Initialize randomly
    ...
    static {
        try {
            UNSAFE = getUnsafe();
            Class<?> sk = AbstractStriped64.class;
